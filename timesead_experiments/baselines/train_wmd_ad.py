...
from timesead_experiments.utils import data_ingredient, load_dataset, training_ingredient, make_experiment, \
    make_experiment_tempfile, serialization_guard, get_dataloader
from timesead.models.baselines import WMDAnomalyDetector
...
def get_test_pipeline():
    ...
    """
    return {
        'window': {'class': 'WindowTransform', 'args': {'window_size': 32}}
    }
...
def get_anomaly_detector(model, val_loader, detector_params, training, _run, save_detector=True):
    ...
    detector = WMDAnomalyDetector(**detector_params).to(training.device)
